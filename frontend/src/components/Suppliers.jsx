        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl border border-slate-100 w-full max-w-lg overflow-hidden animate-spring-up">
            ...
            <form onSubmit={savePO} className="p-6 space-y-4">
              <div><label htmlFor="po-supplier" className="block text-sm font-semibold text-slate-700 mb-1.5">Supplier</label><select id="po-supplier" value={poForm.supplier} onChange={e => setPOForm({...poForm, supplier: e.target.value})} className="w-full px-3 py-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-100 outline-none text-sm bg-white">{suppliers.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
              <div><label htmlFor="po-desc" className="block text-sm font-semibold text-slate-700 mb-1.5">Order Description</label><input id="po-desc" type="text" value={poForm.description || ''} onChange={e => setPOForm({...poForm, description: e.target.value})} className="w-full px-3 py-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-100 outline-none text-sm" placeholder="e.g. Wireless Headphones x50" required /></div>
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2"><label htmlFor="po-amount" className="block text-sm font-semibold text-slate-700 mb-1.5">Total Amount ($)</label><input id="po-amount" type="number" step="0.01" value={poForm.total_amount || ''} onChange={e => setPOForm({...poForm, total_amount: e.target.value})} className="w-full px-3 py-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-100 outline-none text-sm" required /></div>
              </div>
